## Had previously only been correlating with 75W jet position. This doesn't make sense.
...
png("fig2_jet_temp_corr_pearson_0.1.png",8,13,
...
jet_corr <- data.frame(rowMeans(jetdetrend[,which(jet_long>=west & jet_long<=east)]))
...
for (i in 1:length(mon2detrend)){
  rho[i]<- rcorr(mon2detrend[,i],jetdetrend[,which(jet_long==-75)],type = c("pearson"))$r[1,2]
  pval[i]<- rcorr(mon2detrend[,i],jetdetrend[,which(jet_long==-75)],type = c("pearson"))$P[1,2]
}
...
jet <- jet1[seq(month, 852, 12),which(seq(0,357.5,2.5)>=235 & seq(0,357.5,2.5)<=295)]
jet <- jet[47:71,]#1994-2018: 47:71 #(the dataset is from 1948:2018, length(1948:2018) = 71)
rownames(jet) <- 1994:2018
jet <- lapply(jet, as.character)
jet <- lapply(jet, as.numeric)
jet <- as.data.frame(jet)
...
jet_long <- seq(-125,-65,2.5)
jet_corr <- as.matrix(rowMeans(jetdetrend[,which(jet_long>=-120 & jet_long<=-105)]))
#average jet for 90 to 70
...
# 
for (i in 1:length(mon2detrend)){
  rho[i]<- rcorr(mon2detrend[,i],jet_corr,type = c("pearson"))$r[1,2]
  pval[i]<- rcorr(mon2detrend[,i],jet_corr,type = c("pearson"))$P[1,2]
}
...
plot(rotate3, legend.only = T,col =rb, breaks = c(-0.8,-0.6,-.4,-0.2,0,.2,.4,.6,.8),legend.width=1, legend.shrink=1,legend.args=list(text='r', side=3, line=.25, cex=1), axis.args = list(cex.axis = 1),horiz=F)
