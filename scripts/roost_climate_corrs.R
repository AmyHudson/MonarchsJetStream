...
# columns: yrmin; ymax; index(August; September; Mexico; jet5; jet8; jet9); month(May,Aug,Sep); climvar(tmn, tmx, pre, uwnd, vwnd); latitude; longitude; r; p #; wilksp0.1; wilksp0.5
#
...
for (i in 1:length(mon1df)){
  if (is.na(pval[i]) | pval[i] < 0.05){ # if (is.na(pval[i]) | pvalglobal[i] > 0){
...
Clim_var.nc<- Clim_var.nc[,,which(time[,1] >=2002 & time[,1] <=yrmax)]
time <- time[which(time[,1] >=2002 & time[,1] <=yrmax),]
